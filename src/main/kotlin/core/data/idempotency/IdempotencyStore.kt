...
interface IdempotencyStore {
    ...
    suspend fun acquireLock(key: String, fingerprint: String): Boolean
    suspend fun complete(key: String, statusCode: Int, body: String)
...
class ExposedIdempotencyStore(private val database: Database) : IdempotencyStore {

    override suspend fun find(key: String): IdempotencyRecord? = suspendTransaction(db = database) {
    ...
    override suspend fun acquireLock(key: String, fingerprint: String): Boolean =
        suspendTransaction(db = database) {
            IdempotencyKeysTable.insertIgnore {
    ...
    override suspend fun complete(key: String, statusCode: Int, body: String): Unit =
        suspendTransaction(db = database) {
    ...
    override suspend fun deleteStaleLock(key: String): Unit = suspendTransaction(db = database) {
